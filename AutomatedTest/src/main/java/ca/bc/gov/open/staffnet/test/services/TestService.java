package ca.bc.gov.open.staffnet.test.services;
...
public class TestService {
    ...
    @Value("${test.api-host-identity}")
    private String apiHostIdentity;

    @Value("${test.api-host-biometrics}")
    private String apiHostBiometrics;
    ...
    public void setAuthentication(String templateFileName) throws IOException {
        ...
        while (scanner.hasNextLine()) {
            ...
            if (line.contains("{API_HOST_IDENTITY}")) {
                line = line.replaceAll("\\{API_HOST_IDENTITY}", apiHostIdentity);
            }
            if (line.contains("{API_HOST_BIOMETRICS}")) {
                line = line.replaceAll("\\{API_HOST_BIOMETRICS}", apiHostBiometrics);
            }
            writer.append(line + "\n");
        }
        writer.flush();
